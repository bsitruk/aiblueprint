...
Never edit source or tests while a test watcher is running in the checkout. Use
the test commands documented in the repository's `AGENTS.md`, `CLAUDE.md`, or
package scripts.

1. Run the smallest owner and sibling tests with the repository's test runner,
   filtered to the affected paths.
...
4. Run the changed-files gate required by repository policy (lint, typecheck,
   and the CI checks those paths trigger).
5. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
6. After final audit edits, get an independent review of the deleted coverage
   against the remaining keepers.
